...
export TRAIN_NUMS=(250)
export train_times=(1)
# export task_ls=("liangzid/claude3_chat3.3k")
export task_ls=("liangzid/claude3_short256")
# export msl=1500
export msl=256
# export train_taskls=("LoRD-VI" "vanilla" "LoRD-VIII" "LoRD-VII" "LoRD-IX")
export train_taskls=("LoRD-VI" "LoRD-VIII" "LoRD-VII" "LoRD-IX")
...
######################################################################
	...
	if [[ "${train_task}" == "vanilla" ]]; then
	    export fmp="${save_path}___finally/"
	else
	    export fmp="${save_path}___period250/"
	fi

	evaltasks=arc_challenge
	export fewshot_number=25
	$eval --model hf \
	    --model_args pretrained=${pmp},parallelize=True,peft=${fmp},trust_remote_code=True\
	    --tasks ${evaltasks}\
	    --num_fewshot=${fewshot_number}\
	    --device cuda\
	    --batch_size auto

	# ## time consuming: it requires 1:30 hours to execute.
...
echo "RUNNING 1.2.2.longtext_smallmodel.sh DONE."
# 1.2.2.longtext_smallmodel.sh ends here
