...
def load_wmt_query(
    ...
    task_prompt_map = {
        ...
        "de-en": "Translate the sentence from German to English Please. Don't include any additional output",
    ...
    if is_test == 0:
        trainset_text = (
            load_dataset(dataset_name, task_name, split=f"train")
            .shuffle(20240306)
            .to_iterable_dataset()
            .take(train_num)
        )
        ...
        trainset_text = (
            load_dataset(dataset_name, task_name, split=f"test")
            .shuffle(20240306)
            .to_iterable_dataset()
            .take(train_num)
        )
    ...
    return p_idxls, inp_ls, prompts


def load_wmt_nonlabel(
    tokenizer,
    task_name,
    train_num=100,
    max_length=1024,
    ...
    if use_local_model_with_wrmk is None:
        if is_test == 0:
            if tokenizer_name is None:
                ...
                openai_tmp_save_pth += (
                    f"WMTtask_{task_name}-trainNUM_{train_num}_opt.pkl"
                )
            elif "pythia" in tokenizer_name:
                openai_tmp_save_pth += (
                    f"WMTtask_{task_name}-trainNUM_{train_num}_pythia.pkl"
                )
            else:
                openai_tmp_save_pth += f"WMTtask_{task_name}-trainNUM_{train_num}.pkl"
        else:
            if tokenizer_name is None:
                openai_tmp_save_pth += (
                    f"WMTtask_{task_name}-trainNUM_{train_num}.pkl.test"
                )
            elif "opt" in tokenizer_name:
                openai_tmp_save_pth += (
                    f"WMTtask_{task_name}-trainNUM_{train_num}_opt.pkl.test"
                )
            elif "pythia" in tokenizer_name:
                openai_tmp_save_pth += (
                    f"WMTtask_{task_name}-trainNUM_{train_num}_pythia.pkl.test"
                )
            else:
                openai_tmp_save_pth += (
                    f"WMTtask_{task_name}-trainNUM_{train_num}.pkl.test"
        ...
        openai_tmp_save_pth += (
            f"WaterMark-----WMTtask_{task_name}-trainNUM_{train_num}.pkl"
        )
...
def load_wmt_hyprid_gathering(
    ...
    for task_name in hyprid_ls:
        ...
        trainset_text = load_dataset(
            dataset_name, task_name, split=f"train[:{train_num}]"
        )
        trainset_text = (
            load_dataset(dataset_name, task_name, split=f"train")
            .shuffle(20240306)
    ...
    with open(overall_save_p, "wb") as f:
        pickle.dump(
            [
                ntls,
                nprls,
...
def commonly_used_openai_post_process(
    ...
    if not os.path.exists(openai_tmp_save_pth):
        ...
        probsls = []
        iii_bgn = 0
        for iii_bgn, q in tqdm(enumerate(inp_ls), desc="ChatGPT Inference:"):
            qd = [
            ...
            #                                        dtype=torch.long)]
            logits_distr = torch.gather(
                logits_distr, 1, torch.tensor(idx2_dist, dtype=torch.long)
            )

            logits_distr = [logits_distr[i] for i in range(len(logits_distr))]
            ...
            for i, topkdict in enumerate(logprb):
                ...
                topk_subtokenss = [lm_tokenizer.tokenize(x) for x in topk_tokens]
                topk_subidxes = [
                    lm_tokenizer.convert_tokens_to_ids(x) for x in topk_subtokenss
                ]
                topk_logits = [x.logprob for x in topkdict.top_logprobs]
                ...
                for j in range(len(subtokens)):
                    ...
                    idx2_tmp_token_dist = torch.zeros(topk, dtype=torch.long)
                    dist = torch.tensor(topk_logits)
                    # print("dist: ",dist)
        ...
        with open(openai_tmp_save_pth, "wb") as f:
            pickle.dump(
...
def infer_wmt(
    ...
    assert task_name in tasks_we_used
    dataset = (
        load_dataset("wmt16", task_name, split=f"test")
        .shuffle(20240307)
        .to_iterable_dataset()
...
def eval_all():
    ...
    for task in taskls:
        ...
        for m in methodls:
            for itime in train_times:
                ...
                if not os.path.exists(dir_p + res_pth):
                    res_ls = infer_wmt(
                        ckpt, task, dir_p + res_pth, test_set_take_num=100, mnt=64
                    )
                else:
                    # from collections import OrderedDict
                    with open(dir_p + res_pth, "r", encoding="utf8") as f:
    ...
    with open(dir_p + "glue_inference_scores.json", "w", encoding="utf8") as f:
        json.dump(res_dict, f, ensure_ascii=False, indent=4)
    print("OVERALL Save DONE.")
    pprint(res_dict)
...
def eval_ablation():
        ...
        for train_num in train_nums:
            for m in mls:
                ...
                for itime in train_times:
                    ...
                    prefix = "./ablation_ckpts/WMTTT0519"
                    if m == "vanilla" or m == "kd":
                        ...
                        ckpt = prefix + f"{task}{train_num}{itime}{m}___period{num}/"
                    else:
                        ckpt = prefix + f"{task}{train_num}{itime}{m}___period256/"
    ...
    with open(
        dir_p + "Overall__wmt_varytrain_num_inference_scores.json", "w", encoding="utf8"
    ) as f:
        json.dump(res_dict, f, ensure_ascii=False, indent=4)
    ...
    for task in taskls:
        for train_num in train_nums:
            for m in mls:
                ...
                for itime in train_times:
                    # prefix = "./wmt16_ckpts/WMTTT0519"
                    prefix = "./MethodDirectPrompt/WMTTT0519"
                    if m == "vanilla" or m == "kd":
                        ckpt = prefix + f"{task}{train_num}{itime}{m}___finally/"
                    # elif train_num in ["256", "512", "1024"]:
                    #     num = str(int(4 * int(train_num)))
                    #     ckpt = prefix + f"{task}{train_num}{itime}{m}___period{num}/"
                        ...
                        # ckpt = prefix + f"{task}{train_num}{itime}{m}___period256/"
                        # ckpt = prefix + f"{task}{train_num}{itime}{m}___STEPfinally/"
                        # ckpt = prefix + f"{task}{train_num}{itime}{m}temperature0.8___STEPfinally/"
                        ckpt = prefix + f"{task}{train_num}{itime}{m}temperature1.3___STEPfinally/"
                    res_pth = ckpt + base_model_name1 + f"___{task}_wmt_infer_res.json"
                    res_pth = res_pth.replace("/", "__").replace(".", "")
...
def eval_varying_modelsize():
    ...
    with open(
        dir_p + "Overall__wmt_vary_modelsize_inference_scores.json",
        "w",
        encoding="utf8",
    ) as f:
        json.dump(res_dict, f, ensure_ascii=False, indent=4)
